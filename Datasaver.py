...
train=p[:int(0.5*dataloader.length)]
test=p[int(0.5*dataloader.length):]
...
def save(j,root='/jhcnas1/zhoutaichang/prototree_1:1/'):
    ...
    try:
        ...
        data_x_i=np.array(data_x_i)
        # data_x=min_max(data_x)
        data_x_new=[]
        data_x_i_new=[]
    # assert (data_x!=data_x_i_new).all()
        ...
        for channel in range(3):
            ...
            data_x_i_new.append(min_max(zoom(data_x_i[channel], (
            100 / data_x_i[channel].shape[0], 100 / data_x_i[channel].shape[1], 100 / data_x_i[channel].shape[2]))))

    # data_x_new.append(np.zeros((100,100,100))+sick)
    # data_x_i_new.append(np.zeros((100,100,100))+sick)
    #     path=root+str(type)
    ...
    #     path=root+ dataloader.patient_id[j]
        ...
        data_x_i_new = np.array(data_x_i_new).astype(np.float32)
        tensor_x_i = torch.from_numpy(data_x_i_new)
        # torch.save(tensor_x, path  + '.pt')
        # torch.save(tensor_x_i, path + '_i.pt')
        if j in train:
            path=path + dataloader.patient_id[j]

            i=1
            # print(1)
            torch.save(tensor_x, path+'_o' +'.pt')
            torch.save(tensor_x_i, path+'_o' + '_i.pt')
            # print(2)
            axis=['x','y','z']
            for d in range(3):
                torch.save(torch.flip(tensor_x,[d+1]), path+'_'+axis[2-d]  + '.pt')
                torch.save(torch.flip(tensor_x_i,[d+1]), path+'_'+axis[2-d]  + '_i.pt')

            for m in range(3):
                # print(3)

                x=rotation(tensor_x,m,i)
                x_i=rotation(tensor_x_i,m,i)
                torch.save(x, path+str(i)+'_'+str(m) + '_o' + '.pt')
                torch.save(x_i, path+str(i)+'_'+str(m) + '_o' + '_i.pt')
                # for d in range(3):
                #     torch.save(torch.flip(x, [d + 1]), path+str(i)+'_'+str(m) + '_' + axis[2 - d] + '.pt')
                #     torch.save(torch.flip(x_i, [d + 1]),path+str(i)+'_'+str(m) + '_' + axis[2 - d] + '_i.pt')
        else:
            # path = root + 'test/' + dataloader.patient_id[j]
            path = path + dataloader.patient_id[j]
