...
def train(model,data_x,data_y):
    ...
    data_len=len(data_x)
    train_patients=data_x[:int(0.8*data_len)]
    test_patients=data_x[int(0.8*data_len):]
    train_annotation = data_y[:int(0.8*data_len)]
    test_annotation = data_y[int(0.8*data_len):]
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(0,train_len,batch_size):
            ...
            x=train_patients[range(i,min(i+batch_size,train_len))]
        ...
        for i in range(validation_s, validation_e, batch_size):
            x = train_annotation[range(i, min(i + batch_size, validation_e))]
        ...
        if loss < prev_loss:
            torch.save(model.state_dict(), '/jhcnas1/zhoutaichang/conv_model'+str(Hyper.learning_rate)+'.pt')  # save model parameters to files
            prev_loss = loss
            best_epoch=epoch
        ...
        # plt.xlabel('epoch')
        # plt.ylabel('ACC')
        # plt.savefig('/jhcnas1/zhoutaichang/conv_fig' + str(Hyper.learning_rate) + '.png', format='png')
