...
if __name__ == '__main__':
    ...
    for epoch in range(max_epochs):
        ...
        if loss < prev_loss:
            torch.save(model.state_dict(), '/jhcnas1/zhoutaichang/conv_model.pt')  # save model parameters to files
            prev_loss = loss
            best_epoch=epoch
    ...
    # ----------------- plot -------------------
    plt.figure()
    plt.plot(range(0,max_epochs), train_losses,  label='Train')
    plt.plot(range(0,max_epochs), test_losses,  label='Test')
    plt.legend()

    plt.xlabel('epoch')
    plt.ylabel('Loss')
    plt.savefig('/jhcnas1/zhoutaichang/conv_fig.png',format='png')
