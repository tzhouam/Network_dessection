...
if __name__ == '__main__':
    ...
    model = DenseNet(spatial_dims=3,in_channels=3,out_channels=2,block_config=(3,6,6,4)).to(device)
    ...
    for epoch in range(max_epochs):
        ...
        if loss > prev_loss:
            torch.save(model.state_dict(), '/jhcnas1/zhoutaichang/dense_model'+str(Hyper.learning_rate)+'.pt')  # save model parameters to files
            prev_loss = loss
            best_epoch=epoch
        ...
        plt.xlabel('epoch')
        plt.ylabel('Loss')
        plt.savefig('/jhcnas1/zhoutaichang/dense_fig' + str(Hyper.learning_rate) + '.png', format='png')
